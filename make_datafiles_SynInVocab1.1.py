...
#syntactic_tag
# SYNTACTIC_TAG =['S','NP','NN','NNP','NNS','NAC','VP','PP','PRN','POS','PRT'
...
#cuipi syntax in vocab
NO_IN_VOCAB = [SENTENCE_START,SENTENCE_END]
...
def chunk_all():
  ...
  for set_name in ['cnn_wayback_train', 'cnn_wayback_test']:
...
def parsing_stories(tokenized_stories_dir, parsed_stories_dir):
  ...
  for s in stories:
    command = ['java', '-mx50g', '-jar', 'BerkeleyParser-1.7.jar', '-gr','eng_sm6.gr','-inputFile','./'+tokenized_stories_dir+'/'+s,'-outputFile','./'+parsed_stories_dir+'/'+s]
    subprocess.call(command)
    print "files %s is parsered done! " % (s)
...
def write_to_bin(token_glob_dir,parser_glob_dir,out_file, makevocab=False):
  ...
  with open(out_file, 'wb') as writer:
    ...
    #     print "Writing story %i of %i; %.2f percent done" % (idx, num_stories, float(idx)*100.0/float(num_stories))
      ...
      for story_file in glob.glob(parser_glob_dir+'/*'):
        article, abstract = get_art_abs(story_file)
        article_token, abstract_token = get_art_abs('./'+token_glob_dir+'/'+story_file.split('/')[1])

        # Write to tf.Example
        tf_example = example_pb2.Example()
        tf_example.features.feature['article'].bytes_list.value.extend([article])
        tf_example.features.feature['abstract'].bytes_list.value.extend([abstract_token])
      ...
      if makevocab:
        ...
        art_tokens = [t for t in art_tokens if t not in NO_IN_VOCAB] # remove these tags from vocab
...
if __name__ == '__main__':
  ...
  write_to_bin(cnn_tokenized_stories_dir,cnn_preproccessed_stories_dir, os.path.join(finished_files_dir, "cnn_wayback_test.bin"))
  write_to_bin(cnn_tokenized_stories_dir,cnn_preproccessed_stories_dir, os.path.join(finished_files_dir, "cnn_wayback_train.bin"),makevocab=True)
  chunk_all()

  end = time.ctime(time.time())
  print 'start time: %s and end time: %s ' % (start, end)
